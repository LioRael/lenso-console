...
pub struct ContributionGuestClient<'a, H: lenso_guest_sdk::HostImports> {
    capability: lenso_guest_sdk::GuestCapability<'a, H>,
}

impl<'a, H: lenso_guest_sdk::HostImports> ContributionGuestClient<'a, H> {
    pub fn from_context(context: &'a lenso_guest_sdk::GuestContext<H>) -> Result<Self, lenso_guest_sdk::GuestError<serde_json::Value>> {
        context
            .require(CAPABILITY_ID, DESCRIPTOR_VERSION, &[DESCRIBE_CONTRIBUTION_OPERATION], &[], &[])
            .map(|capability| Self { capability })
    }

    pub fn describe_contribution(&self, request: &DescribeContributionRequest) -> Result<DescribeContributionResponse, lenso_guest_sdk::GuestError<DescribeContributionError>> {
        self.capability.request(DESCRIBE_CONTRIBUTION_OPERATION, request)
    }
}

#[derive(Debug, Default)]
pub struct ContributionJsonCodec;

impl lenso_runtime_codec::JsonCapabilityCodec for ContributionJsonCodec {
    fn capability_id(&self) -> &'static str { CAPABILITY_ID }
    ...
    fn request_operations(&self) -> &'static [&'static str] { &[DESCRIBE_CONTRIBUTION_OPERATION] }
    ...
    fn encode_request(&self, operation: &str, request: &dyn std::any::Any) -> Result<serde_json::Value, RuntimeFailure> {
        match operation {
            DESCRIBE_CONTRIBUTION_OPERATION => {
                let value = request.downcast_ref::<DescribeContributionRequest>().ok_or_else(runtime_codec_protocol_failure)?;
    ...
    fn decode_response(&self, operation: &str, value: serde_json::Value) -> Result<Box<dyn std::any::Any>, RuntimeFailure> {
        match operation {
            DESCRIBE_CONTRIBUTION_OPERATION => serde_json::from_value::<DescribeContributionResponse>(value)
                .map(|value| Box::new(value) as Box<dyn std::any::Any>)
    ...
    fn decode_domain_error(&self, operation: &str, value: serde_json::Value) -> Result<Box<dyn std::any::Any>, RuntimeFailure> {
        match operation {
            DESCRIBE_CONTRIBUTION_OPERATION => serde_json::from_value::<DescribeContributionError>(value)
                .map(|value| Box::new(value) as Box<dyn std::any::Any>)
                .map_err(|_| runtime_codec_protocol_failure()),
            _ => Err(runtime_codec_unknown_operation(operation)),
        }
    ...
    fn invoke_host_request(&self, dependency: lenso_kernel::PluginDependencyHandle, operation: String, request: serde_json::Value, context: InvocationContext) -> lenso_runtime_codec::JsonHostRequestFuture {
        match operation.as_str() {
            DESCRIBE_CONTRIBUTION_OPERATION => {
                let request = serde_json::from_value::<DescribeContributionRequest>(request).map_err(|_| runtime_codec_protocol_failure());
                Box::pin(async move {
                    let request = request?;
                    let handle = dependency.typed::<Contribution>()?;
                    match handle.invoke_with_context(DESCRIBE_CONTRIBUTION_OPERATION, context, request).await? {
