...
use lenso_console_app::{ConsoleAppConfig, start_host, store_agent_control_token};
...
async fn run() -> anyhow::Result<()> {
    ...
    let mut config = ConsoleAppConfig::load()?;
    let console_listener = reserve_console_listener(config.address)?;
...
fn reserve_console_listener(
    address: std::net::SocketAddr,
) -> anyhow::Result<std::net::TcpListener> {
    std::net::TcpListener::bind(address).map_err(|error| anyhow::anyhow!(
        "Console cannot listen on {address}: {error}. Choose another port with --port or HTTP_PORT."
    ))
}
