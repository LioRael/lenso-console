...
/* eslint-disable complexity, func-style, no-control-regex -- strict OCI archive validation is intentionally explicit. */

const DIGEST = /^sha256:[0-9a-f]{64}$/u;
const REPOSITORY = /^[a-z0-9]+(?:[._/-][a-z0-9]+)*$/u;

function fail(message) {
  throw new Error(`OCI release artifact: ${message}`);
}
...
function json(bytes, label) {
  try {
    return object(JSON.parse(Buffer.from(bytes).toString("utf-8")), label);
  } catch (error) {
    throw new Error(`OCI release artifact: invalid ${label} JSON`, {
      cause: error,
    });
  }
}
...
export function inspectOciInstallManifest(input) {
  if (
    !REPOSITORY.test(input.registryRepository) ||
    input.registryRepository.includes("..")
  ) {
...
export function inspectOciReleaseArtifact(input) {
  ...
  const layout = json(
    files.get("oci-layout") ?? fail("oci-layout is missing"),
    "oci-layout"
  );
  ...
  const index = json(
    files.get("index.json") ?? fail("index.json is missing"),
    "index"
  );
  if (
  ...
  const manifestBytes =
    blobs.get(root.digest) ?? fail("selected image manifest blob is missing");
  ...
  const dependencies = [
    descriptor(manifest.config, "image config descriptor"),
    ...manifest.layers.map((value, layerIndex) =>
      descriptor(value, `image layer descriptor ${layerIndex}`)
    ),
  ];
  for (const dependency of dependencies) {
    const bytes =
      blobs.get(dependency.digest) ??
      fail(`referenced image blob is missing: ${dependency.digest}`);
  ...
  const labels = object(
    object(config.config, "image config.config").Labels,
    "image config labels"
  );
  if (
    labels["org.opencontainers.image.version"] !== input.version ||
    labels["org.opencontainers.image.revision"] !== input.sourceCommit
  ) {
  ...
  return {
    ...
    blobs,
    installManifestBytes: input.installManifestBytes,
    manifestBytes,
    manifestDigest: root.digest,
    publishedAt: config.created,
    registryRepository: input.registryRepository,
  };
}
