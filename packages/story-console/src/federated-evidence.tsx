  ...
  return (
    <section
      ...
      className={stylexClassName(
        "border-b border-(--line) bg-(--bg-panel-muted) px-3 py-2"
      )}
    >
      <div className={stylexClassName("flex min-w-0 items-center gap-3")}>
        <div className={stylexClassName("min-w-0 flex-1")}>
          <h2
            className={stylexClassName(
              "truncate font-mono text-[11px] font-semibold uppercase tracking-[0.08em] text-(--fg-primary)"
            )}
          >
        ...
        {evidence.tenantId ? (
          <span
            className={stylexClassName(
              "shrink-0 border border-(--line) bg-(--bg-control) px-1.5 py-0.5 font-mono text-[10px] text-(--fg-secondary)"
            )}
          >
      ...
      <div
        className={stylexClassName(
          "mt-2 grid max-h-52 grid-cols-3 gap-2 overflow-auto max-xl:grid-cols-1"
        )}
      >
        <EvidenceDisclosure
        ...
        >
          {workflowKindOrder.map((kind) => {
            ...
            return (
              <div className={stylexClassName("grid gap-1")} key={kind}>
                <h3
                  className={stylexClassName(
                    "font-mono text-[9px] font-semibold uppercase tracking-[0.08em] text-(--fg-quaternary)"
                  )}
                >
                  {workflowKindLabel(kind)}
                </h3>
                <ul className={stylexClassName("grid gap-1")}>
                  {entities.map((entity) => {
                    ...
                    return (
                      <li key={`${entity.kind}:${entity.id}`}>
                        <button
                          ...
                          className={stylexClassName(
                            "grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-2 border border-(--line) bg-(--bg-canvas) px-2 py-1.5 text-left transition hover:border-(--line-strong) disabled:cursor-default"
                          )}
                          disabled={!node}
                          onClick={() => {
                            if (node) {
                        ...
                        >
                          <span className={stylexClassName("min-w-0")}>
                            <span
                              className={stylexClassName(
                                "block truncate text-[11px] font-medium text-(--fg-primary)"
                              )}
                            >
                              {entity.label}
                            </span>
                            <span
                              className={stylexClassName(
                                "block truncate font-mono text-[9px] text-(--fg-quaternary)"
                              )}
                            >
        ...
        <EvidenceDisclosure count={evidence.gaps.length} label="Segment gaps">
          {evidence.gaps.length === 0 ? (
            <p
              className={stylexClassName(
                "font-mono text-[10px] text-(--tone-success-fg)"
              )}
            >
...
function EvidenceDisclosure({
  ...
  return (
    <details
      className={stylexClassName(
        "group border border-(--line) bg-(--bg-panel)"
      )}
      open
    >
      <summary
        className={stylexClassName(
          "flex cursor-pointer list-none items-center justify-between gap-2 border-b border-(--line) px-2 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-[0.06em] text-(--fg-secondary) focus-visible:outline-2 focus-visible:outline-(--focus-ring)"
        )}
      >
        <span>{label}</span>
        <span
          className={stylexClassName(
            "border border-(--line) bg-(--bg-control) px-1 text-(--fg-tertiary)"
          )}
        >
          {count}
        </span>
      </summary>
      <div className={stylexClassName("grid gap-2 p-2")}>{children}</div>
    </details>
  );
}
...
function GapEvidence({ gap }: { gap: FederatedStoryGap }) {
  return (
    <li
      ...
      className={stylexClassName(
        "border border-[var(--tone-warning-border)] bg-[var(--tone-warning-bg)] px-2 py-1.5"
      )}
    >
      <div
        className={stylexClassName("flex items-center justify-between gap-2")}
      >
        <span
          className={stylexClassName(
            "truncate text-[11px] font-medium text-(--fg-primary)"
          )}
        >
          {gap.sourceServiceId}
        </span>
      ...
      <p className={stylexClassName("mt-1 text-[10px] text-(--fg-secondary)")}>
        {gap.detail}
      </p>
      <p
        className={stylexClassName(
          "mt-1 font-mono text-[9px] text-(--fg-tertiary)"
        )}
      >
...
function ReliabilityEvidence({
  ...
  return (
    <li
      ...
      className={stylexClassName(
        "border border-(--line) bg-(--bg-canvas) px-2 py-1.5"
      )}
    >
      <div
        className={stylexClassName("flex items-center justify-between gap-2")}
      ...
      {report ? (
        <>
          ...
          {report.activeDegradedModes.map((mode) => (
            <p
              className={stylexClassName(
                "mt-1 text-[10px] text-(--tone-warning-fg)"
              )}
          ...
          <div className={stylexClassName("mt-1 flex flex-wrap gap-1")}>
            {report.checks.map((check) => (
              <span
                className={stylexClassName(
                  "border border-(--line) bg-(--bg-control) px-1 py-0.5 font-mono text-[9px] text-(--fg-tertiary)"
                )}
                key={check.code}
        ...
        <p
          className={stylexClassName("mt-1 text-[10px] text-(--fg-secondary)")}
        >
...
function EvidenceValues({
  ...
  return (
    <p
      className={stylexClassName(
        "mt-1 line-clamp-2 font-mono text-[9px] text-(--fg-quaternary)"
      )}
    >
      {label}:{" "}
      {entries
...
function EvidenceState({ state }: { state: string }) {
  return (
    <span
      className={stylexClassName(
        "shrink-0 border border-(--line) bg-(--bg-control) px-1 py-0.5 font-mono text-[9px] font-semibold uppercase tracking-[0.04em] text-(--fg-secondary)"
      )}
    >
      {humanize(state)}
    </span>
