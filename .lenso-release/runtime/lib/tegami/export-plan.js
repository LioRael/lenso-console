...
export async function exportReleasePlan(options) {
    ...
    const configUrl = pathToFileURL(join(options.cwd, "tegami.config.mjs"));
    const repositoryConfig = (await import(configUrl.href)).default ?? {};
    const project = tegami({
        ...repositoryConfig,
        cwd: options.cwd,
        plugins: [
