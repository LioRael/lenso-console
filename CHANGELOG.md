...
- e261a88: Keep supervised Agent turns progressing when a browser stops reading its response.
...
- e261a88: Remove the Welcome demonstration from production Console binaries. Activate optional Workspaces only through explicit App defaults or Plugin Root configuration, while retaining standalone Observe and shared contract packages outside the Shell service workspace.
...
- 2e00031: Run Console and Observe OTLP as a Lenso Web App whose Resolved Plan binds Plugin-owned buffered and streaming HTTP Endpoints to Host-owned `lenso.web-ingress` instances. Remove Axum from production service dependencies and make the UI contribution Operation name composable with HTTP providers.
...
- e7fd211: Show the selected Agent's working directory with Agent-scoped task creation and history. Add recorded task Changes with earlier snapshots and truncation indicators, and keep task navigation usable on narrow screens. Directory context requires Lenso Agent 0.1.7; older Agents remain compatible. Accept Session instruction revision events when switching coding Profiles.
