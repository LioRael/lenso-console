...
for (const kind of ['resolve', 'reject']) {
  test(`invalidated event fences late native ${kind}`, async () => {
    let complete;
    const scope = createStorageScope(() => () => new Promise((resolve, reject) => { complete = kind === 'resolve' ? resolve : reject; }), '{}');
    let forwarded = false;
    void scope.storage('accepted', '{}').then(() => { forwarded = true; }, () => { forwarded = true; });
    scope.invalidate();
...
test('unabortable storage has bounded, stable uncertain settlement', async () => {
  let signal;
  const scope = createStorageScope(value => { signal = value; return () => new Promise(() => {}); }, '{}');
...
test('live storage results preserve the normal invocation', async () => {
  const scope = createStorageScope(() => async () => 'null', '{}');
