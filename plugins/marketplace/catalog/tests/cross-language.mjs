...
const fixture = JSON.parse(readFileSync(new URL('./conformance.json', import.meta.url), 'utf8'));
const key = createPublicKey({
  key: Buffer.concat([Buffer.from('302a300506032b6570032100', 'hex'), Buffer.from(fixture.public_key_hex, 'hex')]),
  format: 'der', type: 'spki',
});
const { envelope } = fixture;
const payload = Buffer.from(envelope.payload_base64, 'base64');
const message = Buffer.concat([Buffer.from('lenso.marketplace.snapshot.v1\0'), Buffer.from(envelope.key_id), Buffer.from([0]), payload]);
const signature = Buffer.from(envelope.signature_base64, 'base64');
assert(verify(null, message, key, signature));
